...
- 서버 개발 정본: **PR #146** / `5b46f37ff8b2601555da22831cc6e42896e4dffa`
...
> 구현 완료와 운영 활성화는 구분한다. 이후 개발과 인수인계는 이 문서를 기준으로 하되, 최종 판단은 항상 최신 GitHub 코드/PR 상태를 우선한다.
...
- PageRo는 기존 전용 delivery/SMS 경로를 유지하고 Android Universal pull에서는 `pagero`를 제외한다.
...
| #146 | Google Forms Webhook bridge + Naver/Kakao placeholder 제거 |
...
- #146 `32844885779` — 전체 QA SUCCESS
- production deploy workflows는 skipped.

## 3. Connect 구조 / 최근 보안 정리
...
- body-wide MutationObserver 없음

### #144 — expired-session lifecycle recovery
다음 5개 액션이 세션 만료 401/403에서 로그인 화면으로 복귀한다.
- Meta 연결 해제
- Webhook URL 교체
- Webhook 연결 해제
- API Key 교체
...
공통 `api()`에서 모든 403을 자동 로그아웃으로 처리하지 않는다.

### #145 — auth boundary secret cleanup
`requireLogin()` 시:
- Meta OAuth 임시 session id 삭제
- `webhookSecret` DOM 내용 삭제
- `apiSecret` DOM 내용 삭제
- transient ARIA state 삭제
- Meta page picker/page list 초기화

### #146 — Google Forms bridge
새 서버 저장소/API를 만들지 않고 기존 Generic Webhook을 재사용한다.

사용자 흐름:
1. `/connect → Google Forms`
2. `Google Forms용 Webhook 만들기`
3. Webhook 생성 폼을 `Google Forms` source로 자동 준비
4. 생성 직후 1회 표시되는 URL 복사
5. Google Form의 Apps Script에 제공 스크립트 붙여넣기
6. `<YOUR_CALLTAG_WEBHOOK_URL>`만 실제 URL로 교체
7. `installCallTag()` 1회 실행
8. installable `onFormSubmit` trigger 생성
9. 테스트 응답 제출
10. 기존 Webhook Mapper에서 전화번호 필드 지정
11. 이후 제출은 기존 canonical intake 경로로 처리

Apps Script bridge 계약:
- `ScriptApp.newTrigger(...).forForm(form).onFormSubmit().create()`
- `FormResponse.getItemResponses()`를 `answers` JSON으로 변환
- `response.getId()`를 `Idempotency-Key`로 사용
- `source=google_forms`
- `form_id`, `form_title`, `response_id`, `submitted_at`, `answers` 전송
- 실제 Webhook URL은 코드 저장소/브라우저 storage에 저장하지 않음

Google Forms API의 native watch + Pub/Sub 방식은 가능하지만 현재 제품 구현이 아니다. Pub/Sub 운영 인프라와 watch 갱신 관리가 추가로 필요하므로, 현재는 Apps Script → Generic Webhook bridge를 정식 구현으로 본다.
...
### Google Forms
- 실제 연결 카드/탭 제공
- Google Forms용 Webhook 생성 보조
- Apps Script 자동 제출 bridge 코드 제공
- response id 기반 idempotency
- 기존 Webhook Mapper 재사용
- 전화번호 field mapping 필수
- 별도 D1 migration / 신규 server route 없음
...
### 제외 채널
- **Naver: UI/로드맵 placeholder 제거**
- **Kakao: UI/로드맵 placeholder 제거**
- 현재 공식 범용 Lead 수신 connector 전제로 개발하지 않는다.
...
Google Forms는 위 Generic Webhook API를 재사용하며 별도 route가 없다.
...
- Google Forms 실제 Webhook URL을 공개 저장소/문서/예제에 포함
...
- Google Forms bridge는 기존 Generic Webhook secret/idempotency 계약 재사용
...
- [x] Google Forms Apps Script → Generic Webhook bridge
- [x] Naver/Kakao placeholder 제거
...
- [ ] Direct API/Webhook/Google Forms 실제 운영 수신 확인
...
8. Direct API 실제 문의 수신 확인
9. Generic Webhook sample → mapping → canonical intake 확인
10. Google Forms Apps Script bridge 실제 제출 → Webhook → mapping → canonical intake 확인
11. Meta production env + App Dashboard 설정
12. Meta real Lead Ads 수신 확인
13. Android #100/#101 통합 rollout 준비
14. 실기기 E2E
15. 확인 후 Play rollout
...
- FCM PII 금지
- Naver/Kakao를 준비 중/연결 가능 채널로 다시 노출하지 않음
- Google Forms native API/PubSub가 아직 제품에 구현된 것처럼 표시하지 않음
