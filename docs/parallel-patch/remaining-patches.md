...
Last checked on 2026-05-28 after commit `81b60c1`:
...
- GitHub: pushed to `pc9839a-lgtm/inlet` `main` at commit `81b60c1`.
- Cloudflare Pages: production deployment `0eb13ef5` succeeded for commit `81b60c1`; public URL `https://inlet-8mr.pages.dev/` returns `200`, `<title>Inlet</title>`, and the current asset `index-CBvGgFDN.js`.
...
- Hosted route QA: `INLET_PUBLIC_API_URL=https://inlet-8mr.pages.dev INLET_HOSTED_ROUTE_QA_REQUIRE=1 INLET_HOSTED_ROUTE_QA_WRITE=1 npm run api:hosted:routes:qa` passes with `liveSummary.ready=5`, proving D1-backed public writes for `/api/leads` and `/api/events` plus 403 read protection for `/api/leads`, `/api/leads/export.csv`, and `/api/stats/summary`.
...
- Cloudflare Pages Functions now host `/api/leads`, `/api/leads/export.csv`, `/api/events`, and `/api/stats/summary` against production D1 for the first hosted route slice. Public POST writes create a minimal project/account shell when needed to satisfy D1 foreign keys; GET/read paths still require signed session or API token.
...
3. D1 real runtime smoke and write-side migration
   ...
   - Pages Functions `/api/leads`, `/api/leads/export.csv`, `/api/events`, and `/api/stats/summary` are deployed for the first hosted route slice. Live hosted route QA proves public lead/event writes and protected read endpoints.
   - Remaining work: migrate hosted delivery logs/retry queue, pages/revisions, auth/session, invites/members, ownership transfer, AI draft/key routes, and admin approval routes.
