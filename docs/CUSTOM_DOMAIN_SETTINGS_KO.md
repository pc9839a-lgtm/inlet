...
- 운영 미들웨어는 페이지로 자체 도메인과 `*.pages.dev`를 제외한 외부 호스트만 개인 도메인으로 검사한다.
- 저장된 `integrations.domain.hostname`과 요청 호스트가 일치하면 해당 페이지 slug를 찾는다.
- 개인 도메인의 루트(`/`) 접속은 같은 도메인의 `/{slug}`로 연결한다.
- `www` 유무도 함께 검사한다.
