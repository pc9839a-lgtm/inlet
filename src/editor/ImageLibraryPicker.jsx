...
export default function ImageLibraryPicker({
  ...
  useEffect(() => {
    ...
    listProjectAssets(page, authUser, { kind: 'image', limit: 100 })
      ...
      .catch((loadError) => {
        if (!active) return;
        console.warn('Image library picker load failed:', loadError);
        setError(loadError?.message || '내 이미지를 불러오지 못했습니다.');
      })
      .finally(() => {
  ...
  const loadMore = async () => {
    ...
    setLoadingMore(true);
    try {
      const result = await listProjectAssets(page, authUser, { kind: 'image', cursor, limit: 100 });
      setAssets((current) => uniqueAssets(current, result?.assets || []));
      setCursor(String(result?.cursor || ''));
      setHasMore(!!result?.hasMore);
    } catch (loadError) {
      console.warn('Image library picker pagination failed:', loadError);
      notify(loadError?.message || '이미지를 더 불러오지 못했습니다.', 'error');
    } finally {
  ...
  return createPortal(
    <div className="image-library-picker-backdrop" role="presentation" onMouseDown={(event) => {
      ...
      <section className="image-library-picker" role="dialog" aria-modal="true" aria-label={`${label} 내 이미지에서 선택`}>
        <header className="image-library-picker-head">
          <div>
            <h2>내 이미지에서 선택</h2>
            <p>이 프로젝트에서 이미 발행한 이미지를 다시 사용할 수 있습니다.</p>
          </div>
          <button type="button" className="image-library-picker-close" onClick={onClose} aria-label="내 이미지 닫기">
        ...
        <div className="image-library-picker-body">
          {loading && <div className="image-library-picker-state" role="status">이미지를 불러오는 중입니다.</div>}
          {!loading && error && (
          ...
          {!loading && !error && assets.length === 0 && (
            <div className="image-library-picker-state">
              ...
              <strong>아직 저장된 이미지가 없습니다.</strong>
              <span>이미지를 업로드해 페이지를 발행하면 여기에 자동으로 모입니다.</span>
            </div>
          )}
              ...
              {visibleAssets.map((asset) => {
                ...
                return (
                  <button
                    ...
                    title={asset.fileName || '저장 이미지'}
                  >
                    <span className="image-library-picker-thumb">
                      <img src={asset.downloadUrl} alt="" loading="lazy" />
                    </span>
                    <span className="image-library-picker-name">{asset.fileName || '저장 이미지'}</span>
                    {selected && <span className="image-library-picker-current">현재 이미지</span>}
                  </button>
                );
              })}
            </div>
