...
export function aiDraftToBlocks(draft) {
  ...
  return normalizedDraft.blocks.map((item) => {
    ...
    if (item.type === 'image') return block('image', { mode: 'single', image: item.image || '', gallery: (item.gallery || []).slice(0, 4), galleryLayout: 'slide', galleryGridCount: 4, imageDisplay: 'original', imageHeightPx: 260, imageX: 50, imageY: 50, rounded: true, autoplay: false, interval: 5, galleryShowArrows: true, galleryShowDots: true, caption: item.caption || '' });
