...
const D1_PAGE_JSON_TARGET_BYTES = 1_500_000;
...
async function optimizeDataImage(source, targetBytes) {
  ...
  let smallest = source;
  let smallestBytes = utf8Bytes(source);
  ...
  for (let pass = 0; pass < 5; pass += 1) {
    ...
    for (const quality of [0.82, 0.7, 0.58, 0.46]) {
      ...
      if (candidateBytes <= targetBytes) return candidate;
    }
...
export async function optimizePageForServerSave(page = {}) {
  const initialBytes = pageJsonBytes(page);
  if (initialBytes <= D1_PAGE_JSON_SAFE_BYTES) return page;

  const images = [...dataImageValues(page)]
    .map((source) => ({ source, bytes: utf8Bytes(source) }))
    .sort((a, b) => b.bytes - a.bytes);
  if (!images.length) {
    const error = new Error('페이지 데이터가 서버 저장 한도를 초과했습니다. 불필요한 위젯 내용을 줄인 뒤 다시 저장해주세요.');
    error.status = 413;
    error.details = { code: 'PAGE_DATA_TOO_LARGE', bytes: initialBytes };
    throw error;
  }
  ...
  let estimatedBytes = initialBytes;

  // 큰 이미지부터 필요한 만큼만 줄인다. 예전처럼 모든 이미지를 순차 재인코딩하지 않아
  // 저장 버튼 대기시간과 불필요한 화질 저하를 동시에 줄인다.
  for (const item of images) {
    if (estimatedBytes <= D1_PAGE_JSON_TARGET_BYTES) break;
    try {
      const optimized = await optimizeDataImage(item.source, targetBytes);
      replacements.set(item.source, optimized);
      estimatedBytes -= Math.max(0, item.bytes - utf8Bytes(optimized));
    } catch {
      replacements.set(item.source, item.source);
    }
  }

  const optimized = replacements.size ? replaceDataImages(page, replacements) : page;
