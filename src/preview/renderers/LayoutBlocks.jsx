...
export function RenderTopNav({ block, go }) {
  ...
  useEffect(() => {
    ...
    const check = () => {
      const firstSet = menu.querySelector('.top-menu-set:not(.top-menu-set-copy)');
      const originalWidth = firstSet?.scrollWidth || 0;
      const availableWidth = menu.clientWidth || 0;
      setLoopMenu(originalWidth > availableWidth + 8);
    };
