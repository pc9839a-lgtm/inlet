...
function PageroLandingHomeV2({ onLogin, onSignup }) {
  const heroWidgets = [
    ['이미지', '대표 이미지'],
    ['문의', '카카오 · 전화'],
    ['예약', '날짜 · 시간'],
      ...
      <main className="pagerol-main">
        <section className="pagerol-hero">
          <div className="pagerol-floating-widgets" aria-hidden="true">
            {heroWidgets.map(([label, value], index) => (
...
function PageroLandingHomeSimple({ onLogin, onSignup }) {
  const samples = [
    { label: '상담', title: '무료 상담 신청', action: '상담하기', tone: 'dark', fields: ['이름', '연락처', '문의'] },
    { label: '예약', title: '방문 날짜 선택', action: '예약하기', tone: 'blue', fields: ['날짜', '시간', '연락처'] },
    { label: '이벤트', title: '혜택 받고 신청', action: '신청하기', tone: 'green', fields: ['쿠폰', '성함', '연락처'] },
  ];
  const steps = ['주소 정하기', '방식 선택', '블록 추가', '바로 공유'];
  const purposes = ['광고 상담', '방문 예약', '견적 문의', '이벤트 신청', '강의 모집', '초대장'];
  const blocks = ['이미지', '텍스트', '문의', '예약', '지도', 'FAQ', '타이머', '하단 버튼'];

  return (
    <div className="pagerol-home pagerol-home-v2 pagerol-home-simple">
      <header className="pagerol-nav">
        <a href="/" className="pagerol-logo" aria-label="페이지로 홈">페이지로</a>
        <nav aria-label="메인 메뉴">
          <a href="#build">제작</a>
      ...
      <main className="pagerol-main">
        <section className="pagerol-hero pagerol-simple-hero">
          <div className="pagerol-hero-copy">
            <p>노코드 모바일 랜딩</p>
            <h1>모바일 페이지를<br/>빠르게 만드세요</h1>
            <strong>노코드로 만드는 나만의 랜딩페이지</strong>
          </div>

          <div className="pagerol-mobile-gallery" aria-label="모바일 페이지 예시">
            {samples.map((sample, index) => (
              <article className={`pagerol-sample-card sample-${sample.tone}`} key={sample.label}>
                <div className="sample-topbar"><span/><span/><span/></div>
                <small>{sample.label}</small>
                <b>{sample.title}</b>
                <div className="sample-fields">
                  {sample.fields.map((field) => <i key={field}>{field}</i>)}
                </div>
        ...
        <section className="pagerol-simple-section pagerol-simple-steps" id="build">
          <h2>주소 정하고, 블록 넣고, 공유</h2>
          <div>
        ...
        <section className="pagerol-simple-section" id="cases">
          <h2>필요한 목적만 선택</h2>
          <div className="pagerol-simple-chips">
            {purposes.map((purpose) => <span key={purpose}>{purpose}</span>)}
          </div>
        </section>

        <section className="pagerol-simple-section" id="blocks">
          <h2>자주 쓰는 블록</h2>
          <div className="pagerol-simple-blocks">
            {blocks.map((block) => <span key={block}>{block}</span>)}
          </div>
        </section>

        <section className="pagerol-simple-final">
          <h2>고객에게 보낼 페이지를<br/>먼저 만드세요</h2>
          <button type="button" onClick={onSignup}>시작하기</button>
        </section>
    ...
    <div className="pagerol-home">
      ...
      <main>
        <section className="pagerol-hero">
          ...
          <div className="pagerol-hero-copy">
            <p>상담 · 예약 · 문의 · 접수</p>
            <h1>고객이 들어오는<br/>첫 화면</h1>
            <span className="pagerol-service-mark">WAYZI</span>
            <button type="button" onClick={onSignup}>무료로 시작하기</button>
          </div>
        ...
        <footer className="pagerol-home-footer" aria-label="WAYZI 서비스 정보">
          <div>
            <strong>WAYZI</strong>
            <span>대표 김도윤 · 사업자번호 538-42-01450</span>
          </div>
          <nav aria-label="WAYZI 하단 메뉴">
            <a href="/about">소개</a>
            <a href="/contact">문의</a>
            <a href="/privacy">개인정보처리방침</a>
            <a href="/terms">이용약관</a>
...
export { PublicHome, AuthScreen, DashboardPolished as Dashboard, CreateLandingModal, StartModeOverlay };
