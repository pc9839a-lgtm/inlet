...
for (const [name, source, actions] of [
  ...
  ['account email form', accountEmailFormSource, ['새 이메일 인증', 'current-password', 'one-time-code']],
  ['account settings actions', accountActionsSource, ['changeAuthEmail', "requestEmailVerification(nextEmail, 'email-change')", 'localStorage.setItem(AUTH_KEY']],
