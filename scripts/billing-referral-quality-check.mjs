...
const checks = {
  ...
  'Pagero beta checkout is explicitly inquiry only': ui.billing.includes('웹 자동결제는 준비 중입니다.') && ui.billing.includes("paid ? '가입 문의' : '기본'") && ui.pageroCheckout.includes('현재 웹 자동결제는 준비 중입니다.') && ui.pageroCheckout.includes('가입 문의'),
  'Pagero custom domain beta does not overclaim automatic lifecycle': ui.customDomain.includes('개인 도메인 자동 연결과 SSL 자동 적용은 준비 중입니다.') && ui.customDomain.includes('주소 저장') && ui.customDomain.includes('도입 문의') && ui.pageroCheckout.includes('개인 도메인 자동 연결과 SSL 자동 적용은 현재 준비 중입니다.'),
