...
const productionQaMintAttempts = Math.max(1, Math.min(20, Number(process.env.INLET_PRODUCTION_QA_MINT_ATTEMPTS || 12)));
const productionQaMintDelayMs = Math.max(250, Math.min(10000, Number(process.env.INLET_PRODUCTION_QA_MINT_DELAY_MS || 2500)));
...
async function mintProductionQaSession() {
  let lastStatus = 0;
  let lastCode = '';
  for (let attempt = 1; attempt <= productionQaMintAttempts; attempt += 1) {
    ...
    const propagationCandidate = response.status === 404;
    if (!propagationCandidate || attempt >= productionQaMintAttempts) {
      fail('production QA session mint failed', {
        status: lastStatus,
        code: lastCode,
        attempts: attempt,
      });
    ...
    console.warn(`production QA session mint pending: attempt=${attempt} status=404`);
    await sleep(productionQaMintDelayMs);
  }

  fail('production QA session mint failed', {
    status: lastStatus,
    code: lastCode,
    attempts: productionQaMintAttempts,
  });
}
