...
for (const [name, source, tokens] of [
  ['leads', leads, ['upsertD1Lead', 'listD1Leads', 'findD1LeadsByIntakeSignals', 'insertD1BlockedLeadSubmission', 'publicWrite: true', 'publicProjectShell(project)', "tab: 'inbox'", "url.searchParams.get('dateFrom')", "url.searchParams.get('dateTo')", "url.searchParams.get('channel')", 'getD1PageBySlug', 'getD1LatestPageByProject', 'getD1PublicPageBySlug', 'publicLeadPageContext', 'body.project = project', 'storedPage = await getD1LatestPageByProject', 'normalizeDeliveryPage(inputPage, storedPage || {}, project)', 'const delivery = await sendSavedLeadDelivery', 'deliveryStatus: delivery.status', 'delivery: saved.delivery || delivery', 'LEAD_RATE_LIMITED', 'PUBLIC_POST_HEADERS', "'Access-Control-Allow-Origin': '*'", 'handlePublicPostError', 'trafficAttributionFromSourceUrl', 'utm_source', 'meta: { source:', '중복 접수 정책', '접수는 저장됐지만 알림 전송에 실패했습니다.']],
  ['lead csv', leadCsv, ['listD1Leads', 'month is required for CSV export.', 'text/csv; charset=utf-8', "Content-Disposition", 'csvCell', "tab: 'inbox'", 'parseCsvIds', "url.searchParams.get('ids')", "'Referrer'", "'\\uC720\\uC785 \\uCC44\\uB110'", 'source.sourceUrl || source.url || source.pageUrl', 'source.utm_source', "return cleanFieldLabel(label) || '\\uC785\\uB825\\uAC12'", "'\\uC774\\uB984'"]],
