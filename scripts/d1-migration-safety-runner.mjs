...
export function normalizeMigrationList(value = '') {
  ...
  return source.map((item) => String(item || '').trim()).filter(Boolean).filter((item, index, items) => items.indexOf(item) === index);
}
...
export function evaluateSafetyGate({ mode = 'preflight', branch = '', writeEnabled = false, approval = '', pending = [], expectedPending = [], encryptionSecret = '', liveConfigured = false } = {}) {
  const errors = [];
  ...
  if (mode === 'backup-and-apply') {
    ...
    if (String(encryptionSecret || '').length < MIN_KEY_LENGTH) errors.push(`backup encryption key must be at least ${MIN_KEY_LENGTH} characters`);
  }
...
function outputDir() {
  ...
  if (!relative || relative.startsWith('..') || path.isAbsolute(relative)) throw new Error('D1 migration output directory must remain inside the repository workspace');
  return resolved;
}
...
async function wranglerConfig() {
  ...
  return {
    ...
    databaseId: String(process.env.PAGERO_D1_DATABASE_ID || process.env.INLET_D1_DATABASE_ID || database?.database_id || '').trim(),
...
async function localMigrations() {
  ...
  const names = entries.filter((entry) => entry.isFile() && /^\d+.*\.sql$/i.test(entry.name)).map((entry) => entry.name).sort((a, b) => a.localeCompare(b, 'en'));
  const rows = [];
...
async function d1Query({ accountId, apiToken, databaseId }, sql, params = []) {
  ...
  if (!response.ok || payload.success === false) throw new Error(`Cloudflare D1 query failed (${response.status}): ${redact(JSON.stringify(payload.errors || payload.messages || {}))}`);
...
async function remoteMigrationState(live, migrationsTable) {
  ...
  const safeTable = /^[A-Za-z_][A-Za-z0-9_]*$/.test(migrationsTable) ? migrationsTable : 'd1_migrations';
  const applied = historyAvailable
    ? (await d1Query(live, `SELECT name FROM ${safeTable} ORDER BY id ASC`)).map((row) => String(row.name || '')).filter(Boolean)
    : [];
...
async function run(exe, args, { input = '', env = process.env, allowFailure = false } = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(exe, args, { cwd: ROOT, env, shell: false, stdio: ['pipe', 'pipe', 'pipe'] });
    let stdout = '';
    let stderr = '';
...
async function timeTravelInfo(databaseName) {
  const result = await wrangler(['d1', 'time-travel', 'info', databaseName], { allowFailure: true });
  const bookmark = bookmarkFromText(`${result.stdout}\n${result.stderr}`);
  return {
    available: result.code === 0 && Boolean(bookmark),
    bookmark,
    commandExitCode: result.code,
    note: result.code === 0 ? 'Time Travel information captured.' : 'Time Travel information was unavailable; encrypted SQL export remains required.',
  };
}
...
async function validateSqlExport(filePath) {
  ...
  if (!/(?:CREATE\s+TABLE|INSERT\s+INTO|PRAGMA)/i.test(sample)) throw new Error('D1 export does not contain recognizable SQL backup content');
...
async function encryptSql(plainPath, encryptedPath, secret) {
  ...
  await run('openssl', ['enc', '-aes-256-cbc', '-salt', '-pbkdf2', '-iter', '200000', '-md', 'sha256', '-in', plainPath, '-out', encryptedPath, '-pass', 'env:PAGERO_D1_BACKUP_ENCRYPTION_KEY'], { env });
  const info = await stat(encryptedPath);
  const hmacKey = createHash('sha256').update(`pagero-d1-backup-hmac:${secret}`).digest();
  return { bytes: info.size, sha256: await hashFile(encryptedPath), hmacSha256: await hashFile(encryptedPath, hmacKey), cipher: 'aes-256-cbc', kdf: 'pbkdf2-sha256-200000' };
}
...
function rollbackInstructions({ databaseName, encryptedFile, manifestFile, bookmark }) {
  ...
  if (bookmark) rows.push('', 'Preferred point-in-time command after separate approval:', `npx wrangler d1 time-travel restore ${databaseName} --bookmark=${bookmark}`);
  rows.push('', 'Decrypt only in an isolated workspace:', `openssl enc -d -aes-256-cbc -pbkdf2 -iter 200000 -md sha256 -in ${encryptedFile} -out pagero-d1-restore.sql -pass env:PAGERO_D1_BACKUP_ENCRYPTION_KEY`, '', 'Restore into a disposable D1 database first. Never import the export directly into production without a separately reviewed plan.');
  return `${rows.join('\n')}\n`;
}
...
async function createEncryptedBackup({ databaseName, output, encryptionSecret, baseManifest }) {
  ...
  try {
    await wrangler(['d1', 'export', databaseName, '--remote', '--output', plainPath, '--skip-confirmation']);
    ...
    await writeFile(rollbackPath, rollbackInstructions({ databaseName, encryptedFile: path.basename(encryptedPath), manifestFile: path.basename(manifestPath), bookmark: timeTravel.bookmark }), 'utf8');
...
async function applyMigrations(databaseName) {
  await wrangler(['d1', 'migrations', 'apply', databaseName, '--remote'], { input: 'y\n' });
}
...
async function main() {
  ...
  if (!liveConfigured) {
    const skipped = { ok: !requireLive, status: 'skipped-live', mode, reason: 'Cloudflare account, token, database id, or database name is missing.', localMigrations: localRows, secretValuesIncluded: false };
  ...
  const gate = evaluateSafetyGate({ mode, branch, writeEnabled, approval, pending: pendingBefore, expectedPending, encryptionSecret, liveConfigured });
  if (mode === 'backup-and-apply' && !before.historyAvailable) gate.errors.push(`${config.migrationsTable} history table is unavailable`);
  ...
  const backup = await createEncryptedBackup({ databaseName: config.databaseName, output, encryptionSecret, baseManifest });
  let applyError = '';
  try {
  ...
  const finalManifest = {
    ...
    migrationApply: { attempted: true, error: applyError || null, remoteAppliedMigrationsAfter: after.applied, pendingMigrationsAfter: pendingAfter, expectedApplied },
  };
...
if (invoked === import.meta.url) {
  main().catch(async (error) => {
    const output = (() => { try { return outputDir(); } catch { return path.join(ROOT, OUTPUT_DIR_NAME); } })();
    await mkdir(output, { recursive: true }).catch(() => {});
    const failure = { ok: false, status: 'failed-live', mode: String(process.env.INLET_D1_MIGRATION_MODE || 'preflight'), error: redact(error?.message || error), secretValuesIncluded: false };
    await writeJson(path.join(output, 'd1-migration-failure.json'), failure).catch(() => {});
