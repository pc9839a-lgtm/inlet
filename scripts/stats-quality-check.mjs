...
assert(statsPanel.includes('PERIOD_OPTIONS') && statsPanel.includes('stats-v4-period-tabs'), 'stats panel should expose 1/7/14/30 day period tabs');
assert(statsPanel.includes('type="month"'), 'stats panel should expose month-only filter control');
assert(statsPanel.includes('stats-v4-trend') && statsPanel.includes('<polyline'), 'stats panel trend should render as a line chart');
assert(statsPanel.includes('\uC0C1\uC138 \uD1B5\uACC4') && !statsPanel.includes('\uC6D4\uAC04 \uCD94\uC774') && !statsPanel.includes('\uBC29\uBB38\u00B7\uC811\uC218'), 'stats trend should keep a compact accessible label');
...
assert(/Math\.max\(\d+,\s*hover\.y\)/.test(statsPanel), 'stats chart tooltip should keep top clearance for high data points');
