...
assert(statsPanelCss.includes('body .builder-shell .left-workspace:has(.stats-panel)') && statsPanelCss.includes('overflow-x: clip') && statsPanelCss.includes('overflow-y: clip') && statsPanelCss.includes('overflow-anchor: none'), 'stats filters should prevent the outer workspace from becoming a second scroll container');
...
console.log(JSON.stringify({ ok: true, checks: 70 }, null, 2));
