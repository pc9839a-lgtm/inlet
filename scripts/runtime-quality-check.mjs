...
assert(dashboardScreen.includes('rememberAccountProjectAccess(item)') && accountProjectAccess.includes('rememberedProjects') && pageRepository.includes('delete serverPage.__accountProjectAccess') && pageRepository.includes('if (hasAccountProjectAccess(page)) return false;'), 'Authorized dashboard pages must preserve delegated project context without leaking internal markers or creating account-owned duplicates');
