...
assert(inboxPanel.includes('function enforceFreeEmailIntegration') && inboxPanel.includes('lockedAccountEmail(authUser, page, integrations)') && inboxPanel.includes('locked-email-value') && inboxPanel.includes('계정 이메일로 고정됨') && inboxPanel.includes('lockedToAccount: true'), 'Free plan email alerts must render a non-editable account email in the Inbox UI');
