...
assert(/const saveNow = async[\s\S]*?const nextPage = pageForAccountSave\(sourcePage\);[\s\S]*?setPage\(nextPage\);[\s\S]*?saveLocalJson\(STORAGE_KEY, nextPage, '페이지'\);/.test(app), 'App saveNow must update the visible page and local snapshot from the same account-safe object before server persistence');
