...
assert(app.includes('const pageForAccountSave = (sourcePage = null)') && app.includes('const basePage = sourcePage || latestPageRef.current || page') && app.includes('const normalized = normalizePageForSave(normalizeFreeEmailIntegrations(basePage))') && app.includes("const currentSlug = normalized.slug || defaultPage.slug || 'my-page'") && app.includes('const context = projectContext({ ...normalized, slug: currentSlug }, authUser)') && app.includes('slug: currentSlug') && app.includes('projectId: context.projectId') && app.includes('ownerId: context.ownerId') && !app.includes('const nextSlug = createUniquePageSlug(currentSlug, authUser)'), 'App saves must enforce free email locking while preserving the latest user-selected page URL');
assert(app.includes('const latestPageRef = useRef(page)') && app.includes('const localPageMutationRef = useRef(0)') && app.includes('const loadMutation = localPageMutationRef.current') && app.includes('if (localPageMutationRef.current !== loadMutation) return;') && app.includes('commitLocalPageDraft') && !app.includes('createUniquePageSlug(serverPage.slug || slug, authUser)'), 'Account page load must ignore stale server responses without auto-changing user page URLs');
assert(!app.includes('shouldAutoReplaceSlug') && app.includes("return { ok: true, slug: safeSlug, message: '현재 페이지 주소를 그대로 사용합니다.' }"), 'URL checks must allow the current page URL and must not auto-replace template/default slugs');
...
assert(settingsPanel.includes('onCheckUrl') && settingsPanel.includes("const currentBasic = { title: page.title || '', slug: page.slug || '' }") && settingsPanel.includes('const check = await onCheckUrl?.({ slug })') && settingsPanel.indexOf('const check = await onCheckUrl?.({ slug })') < settingsPanel.indexOf('updatePage({ title, slug })') && settingsPanel.includes("notify(check.message || '이미 사용 중인 페이지 주소입니다.', 'error')"), 'Settings page URL save must validate duplicate slugs before mutating local page state');
assert(/const persistStyleNow = async[\s\S]*?const nextPage = pageForAccountSave\(\{[\s\S]*?\.\.\.\(latestPageRef\.current \|\| page\),[\s\S]*?theme:[\s\S]*?blocks:/.test(app), 'Style saves must use the latest account-safe page save preparation');
assert(/const saveNow = async[\s\S]*?const nextPage = pageForAccountSave\(sourcePage\);[\s\S]*?result = await persistPage\(nextPage, authUser, \{ tab, expectedUpdatedAt \}\);[\s\S]*?const savedPage = result\?\.page \? savedPageFromResult\(nextPage, result\.page\) : nextPage;[\s\S]*?setPage\(savedPage\);[\s\S]*?saveLocalJson\(STORAGE_KEY, savedPage, '페이지'\);/.test(app), 'App saveNow must update the visible page and local snapshot only after successful persistence');
assert(app.includes('const savedPageFromResult = (localPage, serverPage = null)') && app.includes('integrations: serverPage.integrations || localPage.integrations') && app.includes('const savedPage = savedPageFromResult(nextPage, result.page)') && app.includes('const savedPage = savedPageFromResult(safePage, result.page)'), 'App saves must merge server metadata onto the page just sent instead of replacing current widget edits with the response body');
