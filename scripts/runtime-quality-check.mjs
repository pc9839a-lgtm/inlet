...
assert(/const persistStyleNow = async[\s\S]*?normalizePageForSave\(\{[\s\S]*?\.\.\.normalizeFreeEmailIntegrations\(page\)/.test(app), 'Style saves must also enforce free plan email alert recipient from the account email');
