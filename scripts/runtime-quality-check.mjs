...
assert(app.includes('const pageForAccountSave = (sourcePage = page)') && app.includes('const normalized = normalizePageForSave(normalizeFreeEmailIntegrations(sourcePage))') && app.includes('shouldAutoReplaceSlug(currentSlug)') && app.includes('const nextSlug = createUniquePageSlug(currentSlug, authUser)') && app.includes('projectId: context.projectId') && app.includes('ownerId: context.ownerId'), 'App saves must enforce free email locking and replace starter URLs with account-owned page URLs');
assert(app.includes('if (accountPageLoadRef.current !== loadKey) return;') && app.includes('if (shouldAutoReplaceSlug(serverPage.slug || slug))') && app.includes('const nextSlug = createUniquePageSlug(serverPage.slug || slug, authUser)'), 'Account page load must ignore stale server responses and migrate starter server slugs before editing');
