...
assert(inboxPanel.includes('function enforceFreeEmailIntegration') && inboxPanel.includes('lockedAccountEmail(authUser, page, integrations)') && inboxPanel.includes('readOnly={emailLocked}') && inboxPanel.includes('onBeforeInput') && inboxPanel.includes('lockedToAccount: true'), 'Free plan email alerts must stay locked to the account email in the Inbox UI');
