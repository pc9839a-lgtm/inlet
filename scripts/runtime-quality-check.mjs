...
assert(widgetStylePanels.includes('showBackground = true') && widgetStylePanels.includes('showShadow = true') && widgetStylePanels.includes('label="배경 직접 지정"') && widgetStylePanels.includes('label="그림자 추가"'), 'shared surface controls must clearly describe optional background and shadow overrides');
assert((widgetStylePanels.match(/showBackground=\{false\} showShadow=\{false\}/g) || []).length === 8, 'theme-owned card, schedule, timer, activity, map, FAQ, search, and code panels must hide redundant shared surface toggles');
assert((widgetStylePanels.match(/align=\{false\}/g) || []).length === 6, 'timer, activity, map, FAQ, search, and code panels must not expose ineffective shared alignment controls');
assert(widgetStyleOptionsCss.includes('margin-top: var(--widget-margin') && widgetStyleOptionsCss.includes('margin-bottom: var(--widget-margin') && widgetStyleOptionsCss.includes('border-radius: var(--widget-radius'), 'shared vertical spacing and corner controls must reach final preview CSS variables');
...
assert(['.landing-section.text.widget-bg-on', '.landing-section.links.widget-bg-on', '.landing-section.download-widget.widget-shadow-on'].every((selector) => widgetStyleOptionsCss.includes(selector)), 'widgets retaining shared surface controls must consume them in preview CSS');
...
assert(widgetStyleOptionsCss.includes('.landing-section.text.is-bold p') && widgetStyleOptionsCss.includes('.landing-section.text.is-underline p'), 'text bold and underline controls must affect body copy as well as the title');
