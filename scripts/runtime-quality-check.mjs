...
assert(inboxPanel.includes('function enforceFreeEmailIntegration') && inboxPanel.includes('lockedToAccount: true') && inboxPanel.includes('무료 플랜은 계정 이메일로 알림을 받습니다.'), 'Free plan email alerts must stay locked to the account email in the Inbox UI');
assert(app.includes('const normalizeFreeEmailIntegrations') && app.includes('lockedToAccount: true') && app.includes('to: accountEmail'), 'App saves must enforce free plan email alert recipient from the account email');
