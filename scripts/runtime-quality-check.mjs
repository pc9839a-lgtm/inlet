...
assert(dashboardScreen.includes('fetchAccountPages(user)') && dashboardScreen.includes('deleteAccountPage(item, user)') && dashboardScreen.includes('landing-delete-btn'), 'Dashboard must list account-scoped pages and expose page deletion');
assert(dashboardScreen.includes('if (user?.session) return accountPages;'), 'Authenticated dashboards must never expose a persisted page from another account while the scoped server list is loading');
assert(projectsApi.includes('projects.owner_account_id = ?') && projectsApi.includes('project_members.account_id = ?') && projectsApi.includes("project_members.role IN ('manager', 'client_admin')") && projectsApi.includes("NOT IN ('archived', 'deleted')"), 'Project listing must stay isolated to the signed-in owner or an active authorized manager/client member');
assert(pageApi.includes("request.method === 'DELETE'") && pageApi.includes("SET status = 'archived'") && pageApi.includes('authorizeProject(request, env, project, { write: true'), 'Page deletion must require write authorization and archive the target project');
...
assert(workspaceRouteGuards.includes('export function initialWorkspaceOpen') && workspaceRouteGuards.includes('/^\\/app(?:\\/|$)/') && workspaceRouteGuards.includes('/^\\/(?:dashboard|account)(?:\\/|$)/'), 'workspace open state must follow explicit app/dashboard/account routes before persisted state');
...
assert(app.includes("import { initialWorkspaceOpen, isProtectedWorkspacePath, routeUsesWorkspaceTabs as shouldUseWorkspaceTabs } from './runtime/workspaceRouteGuards.js'"), 'App must use the shared workspace route guards');
assert(app.includes('const [workspaceOpen, setWorkspaceOpen] = useState(() => initialWorkspaceOpen('), 'App must initialize workspace visibility from the explicit internal route before persisted state');
...
assert(!persistStyleSaveAction.includes('latestPageRef.current = nextPage') && !persistStyleSaveAction.includes('setPage(nextPage)') && /result = await persistPage\(nextPage, authUser, \{ tab: 'style', expectedUpdatedAt, saveMode: 'update-existing', verifyPublic: false \}\);[\s\S]*?commitSavedPageResult\(\{/.test(persistStyleSaveAction), 'style saves must update visible page only after successful persistence');
...
assert(widgetStyleEditors.filter((source) => !source.includes('export default function TimerEditor')).every((source) => source.includes("label: '스타일'")), 'registered widget editors except the intentionally fixed timer must expose an explicit style tab');
...
assert(!widgetStylePanels.includes('TimerStylePanel') && !widgetStyleEditors.find((source) => source.includes('export default function TimerEditor'))?.includes("id: 'style'"), 'timer editor must keep one fixed base design without dead theme or movement controls');
...
assert(!inboxPanel.includes('window.prompt(') && inboxPanel.includes('inbox-ops-copy-fallback') && inboxPanel.includes('setCopyFallback({ id: lead.id, text })'), 'Inbox lead copy fallback must stay inside the app instead of using browser prompts');
...
assert(pageSaveOptimizer.includes('D1_PAGE_JSON_SAFE_BYTES = 1_750_000') && pageSaveOptimizer.includes('DATA_IMAGE_TOTAL_TARGET_BYTES = 600_000') && pageSaveOptimizer.includes('optimizePageForServerSave') && pageRepository.includes('clientPage: pageWithContext'), 'Oversized embedded images must be optimized below the D1 write ceiling and committed back to the editor state');
assert(d1Adapter.includes('D1_PAGE_JSON_MAX_BYTES = 1_850_000') && d1Adapter.includes('assertD1PageJsonSize(row.page_json)'), 'D1 page and revision writes must reject oversized JSON before issuing a database mutation');
assert(app.includes('pc9839a@naver.com') && localServer.includes('pc9839a@naver.com') && adminSummary.includes('pc9839a@naver.com') && adminSummary.includes('...DEFAULT_MASTER_EMAILS, ...emails'), 'Platform master allowlists must include the requested account across browser, server, and Pages Functions runtimes');
...
assert(pageRepository.includes('const pageWithContext = normalizePageForSave({') && pageRepository.includes('projectId: context.projectId') && pageRepository.includes('ownerId: context.ownerId') && pageRepository.includes('page: pageForServerWrite(pageWithContext)') && pageRepository.includes('accountOwnedPageForRetry(pageWithContext, authUser)'), 'Page repository must persist the same project/owner context that it uses for authorization');
assert(dashboardScreen.includes('const editorPage = { ...selectedPage, __accountProjectAccess: true }') && dashboardScreen.includes('rememberAccountProjectAccess(editorPage)') && accountProjectAccess.includes('rememberedProjects') && pageRepository.includes('delete serverPage.__accountProjectAccess') && pageRepository.includes('if (hasAccountProjectAccess(page)) return false;'), 'Authorized dashboard pages must preserve delegated project context without leaking internal markers or creating account-owned duplicates');
assert(landingRenderer.includes('navigator.share') && landingRenderer.includes('navigator.clipboard?.writeText') && landingRenderer.includes('page-share-button') && !landingRenderer.includes('bottom-share-button') && !bottomBarBasicSection.includes('shareEnabled') && shareOptionsCard.includes('page.share?.enabled') && pageModel.includes("share: { enabled: true, position: 'top-right', display: 'icon' }") && bottomShareCss.includes('.page-share-button.public-share-button'), 'Page sharing must be independent from the bottom bar, open the mobile share sheet, and copy the public URL on desktop');
...
assert(settingsSection.includes("import SettingsActionBar from './SettingsActionBar.jsx'") && settingsSection.includes('const ACTION_LABELS = {') && settingsSection.includes('primaryLabel={locked ? editLabel || labels[1] : saveLabel || labels[0]}') && settingsSection.includes('onPrimary={locked ? onEdit : onSave}'), 'Settings sections must expose explicit save/edit actions through the shared action bar');
assert(settingsAdvancedGroup.includes("activeSection === 'seo'") && settingsAdvancedGroup.includes("activeSection === 'tracking'") && settingsAdvancedGroup.includes("activeSection === 'conversion'") && settingsAdvancedGroup.includes("activeSection === 'duplicate'"), 'Settings advanced group must route each advanced section explicitly');
assert(settingsPanelCss.includes('.settings-section-body{padding:16px') && settingsPanelCss.includes('display:grid;gap:14px'), 'Settings section bodies must keep a readable grid layout');
assert(settingsPanelCss.includes('.settings-grid,.account-settings-grid{display:grid') && settingsPanelCss.includes('@media(max-width:1180px){.settings-grid,.account-settings-grid{grid-template-columns:1fr}}'), 'Settings forms must remain responsive without relying on obsolete section-state controls');
