...
assert(projectsApi.includes('projects.owner_account_id = ?') && projectsApi.includes('project_members.account_id = ?') && projectsApi.includes("NOT IN ('archived', 'deleted')"), 'Project listing must stay isolated to the signed-in owner or active project member');
