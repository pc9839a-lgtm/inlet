...
assert(app.includes('const pageForAccountSave = (sourcePage = page)') && app.includes('const normalized = normalizePageForSave(normalizeFreeEmailIntegrations(sourcePage))') && app.includes("const currentSlug = normalized.slug || defaultPage.slug || 'my-page'") && app.includes('const context = projectContext({ ...normalized, slug: currentSlug }, authUser)') && app.includes('slug: currentSlug') && app.includes('projectId: context.projectId') && app.includes('ownerId: context.ownerId') && !app.includes('const nextSlug = createUniquePageSlug(currentSlug, authUser)'), 'App saves must enforce free email locking while preserving the user-selected page URL');
assert(app.includes('if (accountPageLoadRef.current !== loadKey) return;') && /if \(serverPage\) \{[\s\S]*?setPage\(\(current\) => \{[\s\S]*?return normalize\(serverPage\);[\s\S]*?\}\);[\s\S]*?return;[\s\S]*?\}/.test(app) && !app.includes('createUniquePageSlug(serverPage.slug || slug, authUser)'), 'Account page load must ignore stale server responses without auto-changing user page URLs');
...
assert(/const persistStyleNow = async[\s\S]*?const nextPage = pageForAccountSave\(\{[\s\S]*?\.\.\.page,[\s\S]*?theme:[\s\S]*?blocks:/.test(app), 'Style saves must use the shared account-safe page save preparation');
