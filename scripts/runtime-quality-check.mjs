...
assert(pageSaveOptimizer.includes('D1_PAGE_JSON_SAFE_BYTES = 900_000') && pageSaveOptimizer.includes('DATA_IMAGE_TOTAL_TARGET_BYTES = 600_000') && pageSaveOptimizer.includes('optimizePageForServerSave') && pageRepository.includes('clientPage: pageWithContext'), 'Oversized embedded images must be optimized below the D1 write ceiling and committed back to the editor state');
assert(d1Adapter.includes('D1_PAGE_JSON_MAX_BYTES = 1_000_000') && d1Adapter.includes('assertD1PageJsonSize(row.page_json)'), 'D1 page and revision writes must reject oversized JSON before issuing a database mutation');
