...
assert(pageSaveAction.includes('pageSaveErrorFeedback') && persistStyleSaveAction.includes('pageSaveErrorFeedback'), 'page and style saves must share failure feedback');
assert(pageSaveAction.includes('pageSaveSuccessFeedback') && persistStyleSaveAction.includes('pageSaveSuccessFeedback'), 'page and style saves must share success feedback');
...
assert(/const saveNow = async[\s\S]*?const nextPage = pageForAccountSave\(saveSourcePage\);[\s\S]*?result = await persistPage\(nextPage, authUser, \{ tab, expectedUpdatedAt, saveMode: 'update-existing' \}\);[\s\S]*?const savedPage = result\?\.page \? savedPageFromResult\(nextPage, result\.page\) : nextPage;[\s\S]*?setPage\(savedPage\);[\s\S]*?saveLocalJson\(STORAGE_KEY, savedPage, PAGE_SAVE_LABEL\);/.test(pageSaveAction), 'App saveNow must update the visible page and local snapshot only after successful persistence');
assert(pageSaveHelpers.includes('const savedPageFromResult = (localPage, serverPage = null)') && pageSaveHelpers.includes('integrations: serverPage.integrations || localPage.integrations') && pageSaveAction.includes('const savedPage = result?.page ? savedPageFromResult(nextPage, result.page) : nextPage') && persistStyleSaveAction.includes('const savedPage = savedPageFromResult(nextPage, result.page)'), 'App saves must merge server metadata onto the page just sent instead of replacing current widget edits with the response body');
