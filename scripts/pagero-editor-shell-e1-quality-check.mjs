...
assert(css.includes('body .builder-shell.edit-mode-shell .edit-section-tabs') && css.includes('background: #eef1f5 !important;') && css.includes('border: 0 !important;') && css.includes('min-height: 36px !important;') && css.includes('border-color: #d9dde4 !important;') && css.includes('background: #fff !important;') && !css.includes(".edit-section-tabs button[aria-pressed='true'] {\n  background: var(--product-text);"), 'active editor section tabs must visibly own the compact light segmented state and beat legacy dark button rules');
