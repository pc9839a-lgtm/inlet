...
const visualGeometryContracts = [
  ...
  ['public landing bottom CTA matches the 414px preview canvas', files.previewCss.includes('.public-landing-viewport .public-bottom-bar') && files.previewCss.includes('position: fixed') && files.previewCss.includes('width: min(414px, 100vw)') && files.previewCss.includes('max-width: 414px')],
...
['timer renders numbers only', !files.signal.includes('timer-headline') && !files.signal.includes('timer-cta')],
  ['timer editor exposes only repeat and deadline controls', !files.timerEditor.includes('TimerCtaTargetSection') && !files.timerBasic.includes('s.label') && files.timerBasic.includes('repeatMode') && files.timerBasic.includes('endAt')],
...
const styleConsumerContracts = [
  ...
  ['timer fixed base design', !files.widgetStyles.includes('TimerStylePanel') && files.signal.includes('timer-theme-minimal timer-effect-none') && !files.signal.includes('timer-progress-modern')],
