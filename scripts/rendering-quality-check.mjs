...
const visualGeometryContracts = [
  ...
  ['hero title emphasis controls reach final CSS', files.content.includes("s.bold ? 'is-bold' : ''") && files.content.includes("s.underline ? 'is-underline' : ''") && files.previewCss.includes('.landing-section.hero.is-bold h1') && files.previewCss.includes('.landing-section.hero.is-underline h1')],
  ['text emphasis controls override page font themes', files.previewCss.includes('.landing-section.text.is-bold p') && files.previewCss.includes('.landing-section.text.is-underline p') && /\.landing-section\.text\.is-bold[^}]*font-weight:\s*950\s*!important/s.test(files.previewCss) && /\.landing-section\.text\.is-underline[^}]*text-decoration:\s*underline\s*!important/s.test(files.previewCss)],
  ...
  ['topnav sticky toggle supports opt-out', files.layout.includes("'topnav-sticky' : ''") && files.previewCss.includes('.topnav-one-line.topnav-sticky') && files.previewCss.includes('position: relative !important')],
  ...
  ['bottom CTA separates preset and custom colors', files.widgetStyles.includes('label="색상 방식"') && files.landing.includes("const customButtonColor = s.buttonColorMode === 'custom'") && files.previewCss.includes('.public-bottom-bar.bottom-custom-color button')],
