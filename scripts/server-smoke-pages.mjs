...
await runSmoke('server-smoke-pages', async ({ baseUrl }) => {
  const project = { projectId: 'smoke-pages', slug: 'smoke-page' };
  ...
  const saved = await json({ baseUrl }, 'POST', '/api/pages/smoke-page', { project, page });
  assert(saved.res.ok && saved.data.page?.slug === 'smoke-page', 'page save failed');

  const read = await json({ baseUrl }, 'GET', `/api/pages/smoke-page?${query}`);
  assert(read.res.ok && read.data.page?.title === 'Smoke Page', 'page read failed');

  const updated = await json({ baseUrl }, 'POST', '/api/pages/smoke-page', {
    project,
    page: { ...page, title: 'Smoke Page Updated' },
  });
  assert(updated.res.ok && updated.data.page?.title === 'Smoke Page Updated', 'page update failed');

  const guarded = await json({ baseUrl }, 'POST', '/api/pages/smoke-page', {
    project,
  ...
  const conflict = await json({ baseUrl }, 'POST', '/api/pages/smoke-page', {
    project,
    expectedUpdatedAt: updated.data.page.updatedAt,
  ...
  const revisions = await json({ baseUrl }, 'GET', `/api/pages/smoke-page/revisions?${query}`);
  ...
  const revision = await json({ baseUrl }, 'GET', `/api/pages/smoke-page/revisions/${encodeURIComponent(revisionId)}?${query}`);
  ...
  const restored = await json({ baseUrl }, 'POST', '/api/pages/smoke-page/restore', { project, revisionId: oldestRevision });
  assert(restored.res.ok && restored.data.page?.slug === 'smoke-page', 'page restore failed');

  const afterRestore = await json({ baseUrl }, 'GET', `/api/pages/smoke-page/revisions?${query}`);
