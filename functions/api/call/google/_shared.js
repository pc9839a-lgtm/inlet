...
import { pageroAccountStatus } from '../pagero/_account.js';
...
export async function googleAuthorizationUrl(env = {}, input = {}) {
  if (!googleLoginConfigured(env)) {
    throw oauthError('Google 로그인 설정이 필요합니다.', 503, 'GOOGLE_LOGIN_NOT_CONFIGURED');
  }
...
export async function exchangeGoogleCode(code = '', env = {}) {
  if (!googleLoginConfigured(env)) throw oauthError('Google 로그인 설정이 필요합니다.', 503, 'GOOGLE_LOGIN_NOT_CONFIGURED');
...
export async function consumeGoogleLoginTicket(db, ticket = '', env = {}) {
  ...
  const user = await getD1AccountByEmail(db, normalizeEmail(row.email || ''));
  if (!user || String(user.ownerId || user.id || '') !== String(row.owner_id || '')) {
    throw oauthError('Google 로그인 계정을 찾지 못했습니다.', 404, 'GOOGLE_ACCOUNT_NOT_FOUND');
  }
  const publicUser = authUserPublic(user);
  const profile = await getCallProfile(db, publicUser.ownerId);
  const entitlement = await ensurePendingEntitlement(db, publicUser.ownerId);
  ...
  const pageroConnection = await pageroAccountStatus(db, publicUser.ownerId);
  const publicProfile = profilePublic(profile, publicUser);
  return {
    ok: true,
    user: publicUser,
