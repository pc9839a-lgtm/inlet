...
import { unregisterPushDevice } from './_shared.js';
...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    const result = await unregisterPushDevice(db, session.ownerId, input);
    return jsonResponse(request, env, 200, { ok: true, ...result }, CALL_METHODS);
  } catch (error) {
