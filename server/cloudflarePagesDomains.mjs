import { normalizeDomainHostname } from './pageDomainStore.mjs';
...
function normalizeDnsResolverEndpoint(value = '') {
  ...
  if (parsed.pathname !== '/dns-query' || parsed.search || parsed.hash) {
    throw new Error('DNS resolver must be an exact /dns-query endpoint');
  }
...
export function resolveDnsResolverEndpoint(env = {}) {
  const endpoint = normalizeDnsResolverEndpoint(env.INLET_DNS_JSON_RESOLVER_URL || DEFAULT_DNS_RESOLVER);
  if (!allowedDnsResolverEndpoints(env).includes(endpoint)) {
...
export function cloudflarePagesDomainReadiness(env = {}) {
  ...
  const cnameTarget = normalizeDomainHostname(env.INLET_CUSTOM_DOMAIN_CNAME_TARGET || 'inlet-8mr.pages.dev');
  const missing = [];
...
async function cloudflarePagesRequest(env, pathname, options = {}, fetchImpl = globalThis.fetch) {
  ...
  if (!readiness.configured) {
    throw providerError('운영 도메인 연결 설정이 준비되지 않았습니다.', 503, 'DOMAIN_PROVIDER_NOT_CONFIGURED', {
  ...
  try {
    ...
    throw providerError(
      error?.name === 'AbortError' ? '도메인 연결 서버 응답이 지연되고 있습니다.' : '도메인 연결 서버에 접속하지 못했습니다.',
      502,
      error?.name === 'AbortError' ? 'DOMAIN_PROVIDER_TIMEOUT' : 'DOMAIN_PROVIDER_UNREACHABLE',
    );
  } finally {
  ...
  if (!response.ok || payload?.success === false) {
    ...
    throw providerError(
      ...
      { providerCode: provider.code, providerStatus: response.status },
    );
  }
...
export async function getCloudflarePagesDomain(env, hostname = '', fetchImpl = globalThis.fetch) {
  const readiness = cloudflarePagesDomainReadiness(env);
  return cloudflarePagesRequest(env, domainPath(readiness, hostname), {
    method: 'GET',
    allowNotFound: true,
  }, fetchImpl);
}
...
export async function ensureCloudflarePagesDomain(env, hostname = '', fetchImpl = globalThis.fetch) {
  ...
  if (!safeHostname) throw providerError('도메인 주소를 확인해주세요.', 400, 'DOMAIN_INVALID');
  const existing = await getCloudflarePagesDomain(env, safeHostname, fetchImpl);
  if (existing) return existing;
  return cloudflarePagesRequest(env, domainPath(readiness), {
    method: 'POST',
...
export async function deleteCloudflarePagesDomain(env, hostname = '', fetchImpl = globalThis.fetch) {
  ...
  await cloudflarePagesRequest(env, domainPath(readiness, safeHostname), { method: 'DELETE' }, fetchImpl);
  return { deleted: true, missing: false };
}

export async function inspectCustomDomainDns(env, hostname = '', fetchImpl = globalThis.fetch) {
  const safeHostname = normalizeDomainHostname(hostname);
  const target = cloudflarePagesDomainReadiness(env).cnameTarget;
  const checkedAt = new Date().toISOString();
  if (!safeHostname || !target) {
    return { configured: false, type: 'CNAME', host: safeHostname, target, matched: false, answers: [], checkedAt, error: '' };
  }
  if (typeof fetchImpl !== 'function') {
    return { configured: true, type: 'CNAME', host: safeHostname, target, matched: false, answers: [], checkedAt, error: 'DNS 조회 기능을 사용할 수 없습니다.' };
  }
  ...
  try {
    ...
    return { configured: true, type: 'CNAME', host: safeHostname, target, matched: false, answers: [], checkedAt, error: '허용되지 않은 DNS 확인 주소가 설정되었습니다.' };
  }
  ...
  try {
    ...
    const answers = (Array.isArray(payload?.Answer) ? payload.Answer : [])
      ...
      .filter(Boolean);
    return { configured: true, type: 'CNAME', host: safeHostname, target, matched: answers.includes(target), answers, checkedAt, error: '' };
  } catch {
    return { configured: true, type: 'CNAME', host: safeHostname, target, matched: false, answers: [], checkedAt, error: 'DNS 조회를 완료하지 못했습니다. 잠시 후 다시 확인해주세요.' };
  } finally {
    if (timeoutId) clearTimeout(timeoutId);
  }
}
...
export function mapCloudflarePagesDomain(result = {}, dns = {}) {
  ...
  const failed = new Set(['error', 'blocked', 'deactivated']);
  const providerFailed = failed.has(providerStatus) || failed.has(verificationStatus);
  const sslFailed = failed.has(validationStatus);
  const providerReady = providerStatus === 'active' && (!verificationStatus || verificationStatus === 'active');
  ...
  return {
    ...
    domainStatus,
    sslStatus,
    failureReason: domainStatus === 'failed'
